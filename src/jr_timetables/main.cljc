(ns jr_timetables.main
  "Kotodama WASM entrypoint for the Jr_timetables clean-room actor (L4) — Clojure port.
  ...
  facts (`jr_timetables.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jr_timetables")
(def tier "L4")
...
(def entity-specs
  [{:entity "Stop"         :plural "stops"         :id-prefix "jrtimeta_sto"
    :fields [:code :name :lat :lon]                :required [:code :name]
    :coerce {:lat :float :lon :float}              :refs {}}
   {:entity "Route"        :plural "routes"        :id-prefix "jrtimeta_rou"
    :fields [:shortName :longName :mode]           :required [:shortName :longName]
    :coerce {}                                      :refs {}}
   {:entity "Trip"         :plural "trips"         :id-prefix "jrtimeta_tri"
    :fields [:routeId :headsign :serviceId]        :required [:headsign]
    :coerce {}                                      :refs {:routeId "Route"}}
   {:entity "Vehicle"      :plural "vehicles"      :id-prefix "jrtimeta_veh"
    :fields [:routeId :label :lat :lon]            :required [:label :lat]
    :coerce {:lat :float :lon :float}              :refs {:routeId "Route"}}
   {:entity "ServiceAlert" :plural "servicealerts" :id-prefix "jrtimeta_ser"
    :fields [:routeId :cause :effect]              :required [:cause :effect]
    :coerce {}                                      :refs {:routeId "Route"}}
   {:entity "Fare"         :plural "fares"         :id-prefix "jrtimeta_far"
    :fields [:routeId :price :currency]            :required [:price :currency]
    :coerce {:price :float}                         :refs {:routeId "Route"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"jr_timetables.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jr_timetables-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jr_timetables-compat/ready)
